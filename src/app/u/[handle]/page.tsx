    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="bg-[#0E141B] border border-[#1B2531] rounded-xl p-6 mb-6">
        <div className="flex items-start gap-4">
          <div className="w-16 h-16 rounded-full bg-[#1B2531] border-2 border-[#2BE36C]/40 flex items-center justify-center text-2xl font-bold text-[#E8EDF4] shrink-0">
            {handle[0].toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="font-heading font-bold text-xl text-[#E8EDF4]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4 pt-4 border-t border-[#1B2531]">
          <div className="text-center">
            <div className="font-mono text-[#E8EDF4] font-bold text-lg">{profile.reputation}</div>
            <div className="text-xs text-[#8A94B0]">Reputation</div>
          </div>
          <div className="text-center">
            <div className="font-mono text-[#5AC8FF] font-bold text-lg">{rank}</div>
      ...
      {questions && questions.length > 0 && (
        <section className="mb-6">
          <h2 className="font-heading font-semibold text-[#E8EDF4] mb-3">Launches ({questions.length})</h2>
          <div className="space-y-2">
            {questions.map(q => (
              <Link
                key={q.id}
                href={`/q/${q.slug}`}
                className="flex items-center gap-3 bg-[#0E141B] border border-[#1B2531] rounded-xl p-3 hover:border-[#2BE36C]/40 transition-all"
              >
                <OrbitBadge orbit={q.orbit as Orbit} size="sm" />
                <span className="flex-1 text-sm text-[#E8EDF4] truncate">{q.title}</span>
                {q.status === 'landed' && (
      ...
      {missions && missions.length > 0 && (
        <section>
          <h2 className="font-heading font-semibold text-[#E8EDF4] mb-3">Missions ({missions.length})</h2>
          <div className="space-y-2">
            {missions.map(m => {
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{ color: m.status === 'landed' ? '#2BE36C' : '#F5C542' }}
                  >
                    {m.vote_count >= 5 ? m.score : '—'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[#E8EDF4] truncate">{m.title}</p>
                    {q && <p className="text-xs text-[#8A94B0] truncate">{q.title}</p>}
                  </div>
                  {m.status === 'landed' && (
                    <span className="text-[10px] text-[#2BE36C] font-mono border border-[#2BE36C40] rounded-full px-2 py-0.5">LANDED</span>
                  )}
                </Link>
              )
            })}
